...
% under the same feasible point from QGS1. (...\case9mod\data\case9mod_init_feasiblepoint_(fromQGS1).mat)


mpc = loadcase('case9mod');
load('case9mod_init_feasiblepoint_(fromQGS1).mat', 'Xt_z_QGS')
x0 = Xt_z_QGS(1:24);
...
%% mips:
fprintf('---------------- MIPS for case9mod OPF result----------------\n');
...
fprintf('---------------- IPOPT-mumps for case9mod OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
...
fprintf('---------------- IPOPT-ma97 for case9mod OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
    'ipopt.opts.print_level', 0, ...
...
fprintf('---------------- KNITRO- Interior Point Method for case9mod OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
...
%% knitro: SQP Method
fprintf('---------------- KNITRO- SQP Method for case9mod OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
    'opf.ac.solver', 'KNITRO',...
    'verbose',3);

function [results, success, raw] = nlpopf_solver_For_arbitrary_initpoint(x0, om, mpopt)
...
%% author moidfy--- try to select random interior initial points, unless requested not to
if mpopt.opf.start == 4
    [x0, xmin, xmax] = om.params_var();     %% init var & bounds
    s = 1;                      %% set init point inside bounds by s
    lb = xmin; ub = xmax;
    lb(xmin == -Inf) = -1e10;   %% replace Inf with numerical proxies
    ...
    Vmin = max(bus(:, VMIN), 0.5);
    Vm_range = Vmax- Vmin;
    Vm = Vmin + random(nb+1:2*nb).* Vm_range;
    if mpopt.opf.v_cartesian
        V = Vm * exp(1j*Varefs(1));
        x0(vv.i1.Vr:vv.iN.Vr) = real(V);
        x0(vv.i1.Vi:vv.iN.Vi) = imag(V);
...
%% author moidfy--- select input x0
if mpopt.opf.start == 5
    opt.x0 = x0;
end
...
%%-----  run opf  -----
[x, f, eflag, output, lambda] = om.solve(opt);%solve
success = (eflag > 0);

clear; clc;
% This script implements the case9mod_self system 
...
% under the same feasible point (...\case9mod_self\data\case9mod_self_init_feasiblepoint.mat).


mpc = loadcase('case9mod_self');
load('case9mod_self_init_feasiblepoint.mat', 'x0');
...
%% mips:
fprintf('---------------- MIPS for case9mod_self OPF result----------------\n');
...
fprintf('---------------- IPOPT-mumps for case9mod_self OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
...
fprintf('---------------- IPOPT-ma97 for case9mod_self OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
    'ipopt.opts.print_level', 0, ...
...
fprintf('---------------- KNITRO- Interior Point Method for case9mod_self OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
...
mpopt = mpoption(mpopt, ...
    ...
    'knitro.maxit',             3000 ...
     );% opt = knitro_options % ref https://www.artelys.com/app/docs/knitro/3_referenceManual/userOptions.html#useroptions
om = opf_setup(mpc, mpopt);
tic;
[results, success, raw] = nlpopf_solver_For_arbitrary_initpoint(x0, om, mpopt); 
...
%% knitro: SQP Method
fprintf('---------------- KNITRO- SQP Method for case9mod_self OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
    'opf.ac.solver', 'KNITRO',...
    'verbose',3);

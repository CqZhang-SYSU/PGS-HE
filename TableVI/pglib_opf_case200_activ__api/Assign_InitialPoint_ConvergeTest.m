...
[mpc, mpopt] = opf_args('pglib_opf_case200_activ__api');
% Set solver options
mpopt = mpoption; 
...
load('case200_init_feasiblepoint.mat', 'x0')
...
%% mips:
fprintf('---------------- MIPS for 200-bus OPF ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'MIPS', ...
    'opf.start', 1, ...%input initial point
    'verbose', 3,...
...
fprintf('---------------- IPOPT-mumps for 200-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
...
fprintf('---------------- IPOPT-ma97 for 200-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
    'ipopt.opts.print_level', 0, ...
...
fprintf('---------------- KNITRO- Interior Point Method for 200-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
...
%% knitro: SQP Method
fprintf('---------------- KNITRO- SQP Method for 200-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
    'opf.ac.solver', 'KNITRO',...
    'verbose',3);

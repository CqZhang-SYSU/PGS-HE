...
mpc = loadcase('LMBM3_50');
load('LMBM3_50_init_infeasiblepoint.mat','x0');
...
%% mips:
fprintf('---------------- MIPS for 3-bus OPF result----------------\n');
...
fprintf('---------------- IPOPT-mumps for 3-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
...
fprintf('---------------- IPOPT-ma97 for 3-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1, ...
    'opf.ac.solver', 'IPOPT', ...
    'opf.start', 5, ... %input initial point
    'ipopt.opts.print_level', 0, ...
...
%% knitro: SQP Method
fprintf('---------------- KNITRO- Interior Point Method for 3-bus OPF result ----------------\n');
mpopt = mpoption('exp.use_legacy_core', 1,...
    'opf.start', 5,...
